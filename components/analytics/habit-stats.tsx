...
function calculateHabitStats(
  ...
  return habits
    .map((habit) => {
      ...
      const doneCount = habitCheckIns.filter(
        (c) => normalizeCheckInStatus(c.status) === CheckInStatus.Done,
      ).length;
      const partialCount = habitCheckIns.filter(
        (c) => normalizeCheckInStatus(c.status) === CheckInStatus.Partial,
      ).length;
      const skippedCount = habitCheckIns.filter(
        (c) => normalizeCheckInStatus(c.status) === CheckInStatus.Skipped,
      ).length;

      const successRate =
        habitCheckIns.length > 0
          ? Math.round(
              ((doneCount + partialCount * 0.5) / habitCheckIns.length) * 100,
            )
          : 0;
